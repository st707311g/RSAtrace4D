...
class RootNode(Node):
    ...
    def complete_polyline(self):
        ...
        def complete(node1: List[int], node2: List[int]):
            ...
            return np.array(np.stack([np.linspace(node1[i], node2[i], max_dif+1, dtype=np.int32) for i in range(3)])).T.tolist()
...
class RSA_Vector(Node):
    ...
    def load_from_dict(self, trace_dict: dict = {}, file=''):
        try:
            ...
            version, revision = parse_version_string(general_annotations['version'])
            if version > 1: 
